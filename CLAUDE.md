...
**Nombre oficial:** **Tarot Companion** (2026-07-08: la autora revirtió "Velara" → "Tarot Companion" al ser app de uso personal; **conserva el símbolo** sol/luna oro+lavanda del imagotipo Velara, solo cambió el nombre a texto). Aplicado en index.html (title/apple-title/brand → símbolo SVG + `<span class="brand-name">`), manifest (name="Tarot Companion", short_name="Tarot"), strings (`menu.aboutText` ES+EN) y sw.js (`CACHE_VERSION="tarot-companion-v3"`). Historial de naming: "Tarot Companion" → "Velara" (2026-06-25) → "Tarot Companion" (2026-07-08).
...
- [x] `js/uso.js` — "Cómo usar" (bilingüe; desde 2026-08-03 solo práctica + anatomía de carta)
- [x] `js/simbolos.js` — Glosario·Símbolos: estructura del tarot + glosario de 22 símbolos (bilingüe, 2026-08-03)
...
#### Sesión 2026-08-03 — Nueva IA "Tarot/Glosario" + vista Símbolos (estructura + glosario) + guía nutrida
- **Contexto:** la autora pidió nutrir la guía (Mayores/Menores, palo↔elemento, corte como camino, glosario de símbolos) y, al revisar el resultado, decidió **reorganizar la IA**: el material de consulta no debía vivir en la pestaña de práctica.
- **IA final (decisión de la autora):** barra inferior = Inicio · **Tarot** · **Glosario**. **Tarot** (antes "Glosario") = práctica: sub-pestañas **Arcanos** (grid 78) + **Tiradas** (las 4; "iremos agregando tiradas"). **Glosario** (antes "Tiradas") = aprendizaje: sub-pestañas **Cómo leer** + **Símbolos** (nueva) + **Números**. Hashes: `#tarot/arcanos`, `#tarot/spreads`, `#glosario/uso`, `#glosario/simbolos`, `#glosario/numerologia`; deep link `#arcano/:id` abre Tarot·Arcanos.
- **Nueva vista `js/simbolos.js`** (Glosario·Símbolos, bilingüe co-localizada, patrón de uso.js), en formato **visual**: (1) Mayores con **línea de viaje** `0·El Loco ——— el viaje ——— XXI·El Mundo`; (2) palos como **tarjetas 2×2** con acento de grupo + **chip de elemento** (una sola mención — se corrigió la duplicación "Copas·Agua + AGUA" usando nombre de palo local, no `t("suit.x")` que ya trae el elemento) + "su pregunta" (¿Qué siento/tengo/deseo/pienso?); (3) corte como **camino numerado 1→4** (Sota=aprendizaje, Caballero=búsqueda, Reina=maduración, Rey=maestría) con línea conectora; (4) **tabla de correspondencias** palo–elemento–terreno–pregunta; (5) **glosario de 22 símbolos** Waite-Smith con cierre anti-dogma.
- **`js/uso.js` reescrito** = solo práctica: mente → interpretar → **"La anatomía de una carta"** (nueva: cómo desarmar "Cinco de Copas" / "Reina de Bastos" / Mayor con nombre propio, enlaza a Símbolos y Números) → posiciones. Enlaces internos actualizados en uso/spreads/sheet (returnHash default → `tarot/arcanos`).
- **Cambios de soporte:** `index.html` (secciones/tabs/nav reordenados; segGlosario ahora 3 tabs), `router.js` (VIEWS + deep link → tarot), `app.js` (SUBS/SEG_IDS genéricos, tiles de Inicio → `tarot/arcanos` y `glosario/uso`, `initSimbolos` en boot y applyLanguage), `strings.js` (`tab.tarot`, `seg.simbolos`, `home.tarotCard/Sub` y `glosarioCard/Sub` nuevos; fuera `tab.tiradas`/`home.tiradasCard`), CSS nuevo en `components.css` (`.journey`, `.suit-cards/.suit-card`, `.court-path/.path-step`, `.corr-table`, `.gloss-*` — todo con tokens → sirve en las 4 combos paleta×tema), `sw.js` (precache + `js/simbolos.js` **y `js/scroll-lock.js` que faltaba desde antes**; `CACHE_VERSION` → `tarot-companion-v6`).
- **Verificado (Chrome real, servidor local :8146, hard-reload):** nav = Inicio/Tarot/Glosario (ES) y Home/Tarot/Glossary (EN); grid 78 cartas en Tarot·Arcanos; tiradas renderizan; Símbolos completo en ES y EN (sin fugas cruzadas); anatomía presente en Cómo leer; enlaces internos correctos; deep link `#arcano/the-fool` abre Tarot·Arcanos + sheet; prev/next OK (The Fool→The Magician); Escape cierra y vuelve a `#tarot`; **tema claro verificado** (tarjetas blancas, chips y línea de viaje legibles); cero errores de consola; `node --check` OK; paridad de claves **88/88**.
- **Pendiente:** revisión de contenido por la autora + commit. Deploy a Pages sigue pendiente.

#### Sesión 2026-08-07 — Símbolos más explícitos (mapa de la carta + orden de la corte) + scouting de tiradas nuevas
- **Completado en `js/simbolos.js`** (ES+EN): (1) nueva sección **"El mapa de la carta"** entre la tabla y el glosario — lectura espacial de la ilustración: arriba=cielo/mental (por eso el aire vive arriba), abajo=suelo/material (la tierra en la parte baja; qué pisa la figura), primer plano=lo inmediato, fondo=contexto/lo lejano, mirada y cuerpo de la figura=dirección de la energía, contar los objetos=el número de la carta dibujado en escena; cierra enlazando a Números ("el número dice la fase; la escena, cómo se está viviendo"). (2) En la corte, nueva nota **explícita sobre el orden**: el 1→4 numera la *madurez* (Sota abre, Rey culmina), no el rango — en jerarquía el Rey encabeza (respuesta a confusión real de la autora: leyó el 1 como rango). `node --check` OK. SW sigue en `tarot-companion-v6` (sin deploy intermedio).
- **Scouting de tiradas (petición de la autora):** post de Instagram de **@lidapavlova_magic** (libro *"The Library of Questions: Unconventional Tarot Spreads and Journaling Prompts"*, 123 tiradas; el post comparte 8). Leídas con el Chrome de la autora (WebFetch anónimo falla por login-wall; carrusel navegado por `?img_index=N`). Las 8: **August's Gifts** (5, abundancia estacional), **That Feeling** (5, entender una emoción rara), **Roots and Fruits** (8, herencia familiar: raíces/frutos/legado que pesa/legado que ayuda), **Changing Faces** (5, cuando algo cambió en ti), **The Ultimate Guide Through Scary Things** (4, antes de algo importante que asusta), **The Hooded Old Man** (5, introspección narrativa: bosque + anciano encapuchado), **Benevolent Spirits** (5, apoyos invisibles), **Adventure** (4, lúdica: arma/aliado/enemigo/desenlace).
- **⚠️ Derechos:** contenido del libro publicado de Lida Pavlova. La autora aprobó el enfoque **adaptar con redacción propia + crédito de inspiración** (si algún día se publica la app, revisar/licenciar).
- **5 tiradas nuevas implementadas** (elección de la autora: las 4 favoritas + la de agosto generalizada a mensual). Nuevo registro **`SIMPLE_SPREADS`** en `spreads.js` (tiradas de posiciones fijas, data-driven: añadir tirada = añadir entrada; render/sorteo/estado genéricos): **Esa sensación** (5), **Cambio de piel** (5), **El anciano del bosque** (5, narrativa con entrada `story` en cursiva/borde dorado), **Aventura** (4), **El umbral del mes** (5, para abrir cada mes — adaptación generalizada de "August's Gifts"). Todas bilingües con redacción propia; línea de **crédito** al pie de la vista («inspiradas en "The Library of Questions" de Lida Pavlova», clave `credit` en TXT). Estado `simpleIds` conserva las cartas extraídas al cambiar de idioma. CSS: `.slots.n4/.n5` (5ª carta a lo ancho), `.spread-story`, `.spread-credit`. `lead` de la vista ya no dice "cuatro caminos".
- **Verificado (Chrome real, :8147):** las 5 tiradas sortean su número de cartas; cada carta abre el sheet; cambio de idioma por la UI conserva las cartas y re-localiza nombres/roles/historia/crédito (ES↔EN); cero errores de consola; `node --check` OK.

#### Sesión 2026-08-07 (2) — 🚀 DEPLOY A GITHUB PAGES
- **La app está publicada:** **https://leaf-alexia.github.io/tarot-companion/** ✅ (verificada cargando en Chrome).
- **Cambio de cuenta GitHub:** la autora migró a la cuenta **Leaf-Alexia** (el repo viejo `alexia-raccoon/yokai-tarot` quedó inaccesible con la credencial guardada — el push devolvía 403 "denied to Leaf-Alexia"). Ella creó el repo **`Leaf-Alexia/tarot-companion`** ("Just a companion for learning tarot") desde su navegador; la credencial git de Windows ya era de Leaf-Alexia → push directo.
- **Pasos ejecutados:** (1) commit `c41ff48` del trabajo pendiente (IA nueva + simbolos.js + 5 tiradas; **`estudios/` excluida a propósito** — notas personales, no van al repo público). (2) `main` fast-forward a `testing` (traía todo: V3→hoy, incl. borrado de `yokai-tarot.html`). (3) `git remote set-url origin` → repo nuevo; push de `main` y `testing`. (4) Vía Chrome de la autora: repo **privado→público** (requisito de Pages gratis), **default branch master→main** (el repo nació con un `master` inicial de GitHub), **Pages: Deploy from a branch → main / (root)**. (5) Verificado: la URL sirve la app (Inicio con luna + carta del día + tiles Tarot/Glosario).
- **Notas:** queda una rama `master` residual en el remoto (el README inicial del repo vive ahí; `main` no tiene README — inofensivo, se puede borrar o hacerle un README propio después). La carpeta `branding/` es pública (avisado a la autora antes de publicar). Próximos deploys = commit + `git push` a `main` (y **subir `CACHE_VERSION`** en sw.js). Instalación en el teléfono: abrir la URL en Chrome → menú ⋮ → "Agregar a pantalla principal".

#### Sesión 2026-07-08 — Rename inverso a "Tarot Companion" + paleta "Cereza" (selector)
- **Contexto:** siendo ya app personal, la autora pidió (1) volver al nombre **Tarot Companion** conservando el *símbolo* de Velara, y (2) recuperar la paleta cálida del `yokai-tarot`. Decisión: **no reemplazar** la paleta actual, sino **añadir un selector de "Paleta"** (Ciruela / Cereza) en Ajustes, ortogonal al tema claro/oscuro → **4 combinaciones**.
- **Rename (símbolo intacto):** en `index.html` el imagotipo era un SVG con dos `<g>` (símbolo + wordmark "VELARA" vectorial). Se conservó el primer `<g>` (símbolo, recortado con `viewBox="60 56 188 188"`, `aria-hidden`), se **eliminó** el `<g>` del wordmark. **Decisión final de la autora (mismo día): el header muestra SOLO el símbolo, sin texto** — el nombre "Tarot Companion" vive en title/manifest/about, no en el header. CSS `.brand`/`.brand-logo` en `layout.css` (símbolo 28×28). Title/apple-title, `manifest` (name/short_name), `menu.aboutText` (ES+EN) y `sw.js` (comentario + `CACHE_VERSION` `velara-v2` → `tarot-companion-v4`).
- **Paleta "Cereza" (eje nuevo `data-palette`, espejo de `data-deck`):** nuevo `css/themes/cereza.css` con bloques `:root[data-palette="cereza"]` (oscuro, valores base del yōkai: fondo `#1a1018`, acento cereza `#ce4e75`/`#ff7d92`, oro `#d6b873`) y `:root[data-palette="cereza"][data-theme="light"]` (variante pergamino cálido autorada, acento `#B23E62`), + acentos de palo por combinación. Solo **sobreescribe** variables → la paleta "plum" (ciruela/lavanda) queda intacta cuando `data-palette` está ausente. `store.js`: `K_PALETTE`/`getPalette`(def "plum")/`setPalette`. `settings.js`: `META_COLOR` reestructurado a `{paleta:{tema}}`, `updateMetaColor()`, `applyPalette()` (limpia el atributo si "plum"), sección "Paleta" + handler. `app.js`: `applyPalette(getPalette())` en boot. Script **anti-flash** de `index.html` extendido para fijar `data-palette` + theme-color correcto antes de pintar. i18n: `menu.palette`/`palettePlum`/`paletteCereza` (ES+EN).
- **Verificado (Chrome, servidor local):** título/marca = "Tarot Companion" con símbolo; menú muestra "Paleta"; las **4 combos** (Ciruela/Cereza × Oscuro/Claro) pintan correctamente (Cereza oscuro = ciruela-café cálido + acento cereza; Cereza claro = pergamino cálido con contraste); acentos de palo por grupo OK; persistencia tras recarga **sin flash**; `meta theme-color` correcto por combo; `node --check` OK; **paridad de claves ES/EN 87/87**; cero errores de consola.
- **Gotcha de testing (no es bug de la app):** el servidor local de Python no manda `Cache-Control`, así que Chrome cachea los módulos ES con frescura heurística y el SW los sirve rancios; para ver cambios hay que **hard-reload** o usar otro puerto (se probó en `:8140` limpio). En GitHub Pages no aplica (manda headers correctos).
- **Pendiente:** commit de esta sesión + del working tree previo (eliminaciones de mockups/`yokai-tarot.html`, `branding/` sin trackear). Deploy a Pages/teléfono: sigue pendiente.
