...
- **Rename (símbolo intacto):** en `index.html` el imagotipo era un SVG con dos `<g>` (símbolo + wordmark "VELARA" vectorial). Se conservó el primer `<g>` (símbolo, recortado con `viewBox="60 56 188 188"`, `aria-hidden`), se **eliminó** el `<g>` del wordmark y el nombre pasó a **texto real** `<span class="brand-name">Tarot Companion</span>` (serif, hereda `--cream` por tema). CSS `.brand`/`.brand-logo`/`.brand-name` en `layout.css`. Title/apple-title, `manifest` (name/short_name), `menu.aboutText` (ES+EN) y `sw.js` (comentario + `CACHE_VERSION` `velara-v2` → `tarot-companion-v3`).
